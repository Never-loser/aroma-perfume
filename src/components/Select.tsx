...
/**
 * Hand-made, RTL-aware luxury dropdown (replaces native <select>).
 */
    ...
    <div ref={ref} className={`relative ${className}`}>
      ...
      >
        <span className="flex min-w-0 items-center gap-1.5">
          ...
          <span className={`truncate ${current ? "text-zinc-100" : "text-zinc-500"}`}>
            {current?.label ?? placeholder}
          </span>
        </span>
        <ChevronDown
          size={14}
          className={`shrink-0 text-amber-gold transition-transform duration-300 ${open ? "rotate-180" : ""}`}
        />
      </button>
            ...
            return (
              <button
                ...
                className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-right text-xs transition ${
                  active ? "bg-amber-gold/15 text-amber-glow" : "text-zinc-300 hover:bg-white/5"
                }`}
              >
